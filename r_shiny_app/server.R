  ...
  output$projections_plot_ui <- renderUI({
    ...
    observe({
      ...
      output$file_path_text <- renderText({
        parseFilePaths( roots = volumes, input$file)$datapath
      })
    })
    
    observeEvent(input$load_file, {
      filepath <- isolate(parseFilePaths( roots = volumes, input$file)$datapath)
    ...
    mod_file_02encodermvp <- function(filepath){
      ...
      print("all amendments have been added to the files ")
    }
