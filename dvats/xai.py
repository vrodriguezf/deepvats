...
# %% auto 0
__all__ = ['get_embeddings', 'get_dataset_old', 'get_dataset', 'umap_parameters', 'get_prjs', 'plot_projections',
           'plot_projections_clusters', 'calculate_cluster_stats', 'anomaly_score', 'detector',
           'plot_anomaly_scores_distribution', 'plot_clusters_with_anomalies', 'update_plot',
           'plot_clusters_with_anomalies_interactive_plot', 'get_df_selected', 'shift_datetime', 'get_anomalies',
           'get_anomaly_styles', 'InteractiveAnomalyPlot', 'plot_save', 'plot_initial_config', 'ts_plot_interactive']
...
def get_dataset(
    ...
    enc_artifact_train = artifacts_gettr(
        enc_logger.config['train_artifact'], type='dataset'
    )
    if enc_logger.config['valid_artifact'] is not None:
        enc_artifact_valid = artifacts_gettr(
            enc_logger.config['valid_artifact'], type='dataset'
        )
        if print_flag: print("enc_artifact_valid:", enc_artifact_valid.name)
    if print_flag: print("enc_artifact_train: ", enc_artifact_train.name)
    
    if config_emb.dr_artifact is not None:
        dr_artifact = artifacts_gettr(config_emb.dr_artifact)
    else:
        dr_artifact = enc_artifact_train
    if print_flag: print("DR artifact train: ", dr_artifact.name)
    print(dr_artifact.name)
    df = dr_artifact.to_df()
    if print_flag: display(df.head())
    return df, dr_artifact, enc_artifact, enc_learner
