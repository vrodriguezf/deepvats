...
def get_enc_embs_set_stride_set_batch_size(X, enc_learn, stride, batch_size, module=None, cpu=False, average_seq_dim=True, to_numpy=True, print_flag = False):
    ...
    if not cpu:
        if (print_flag): print("get_enc_embs_set_stride_set_batch_size | check size")
        total_emb_size = sum([emb.element_size() * emb.nelement() for emb in embs])
        free_memory = torch.cuda.get_device_properties(0).total_memory - torch.cuda.memory_allocated()
        if (total_emb_size < free_memory):
            if (print_flag): print("get_enc_embs_set_stride_set_batch_size | Fit in GPU => Move all embs to GPU")
            embs=[emb.cuda() for emb in embs]
        else:
            if (print_flag): print("get_enc_embs_set_stride_set_batch_size | Dont fit in GPU => Move all to CPU")
            embs=[emb.cpu() for emb in embs]
    embs = to_concat(embs)
